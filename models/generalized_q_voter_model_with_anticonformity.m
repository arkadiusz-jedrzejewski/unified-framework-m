function model = generalized_q_voter_model_with_anticonformity(q_a, q_c, r)
%   Constructs the transition probabilities for the generalized q-voter
%   model with anticonformity.
%
%   model = generalized_q_voter_model_with_anticonformity(q_a, q_c, r)
...
%       r   :  intensity of conformity relative to anticonformity
%
%   Output: 
%       model : struct with 8 function handles
    ...
    model.X_AB = @(a) a ^ q_a;
    model.Y_BA = @(a) r * a ^ q_c;
    model.Y_AB = @(a) r * (1 - a) ^ q_c;

    % Derivatives 
    model.dX_BA = @(a) - q_a * (1 - a) ^ (q_a - 1);
    model.dX_AB = @(a) q_a * a ^ (q_a - 1);
    model.dY_BA = @(a) r * q_c * a ^ (q_c - 1);
    model.dY_AB = @(a) - r * q_c * (1 - a) ^ (q_c - 1);
end